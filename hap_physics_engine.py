...
def main():
    ...
    m_bl_air = 0.000000001  # current air mass in the ballonet               (kg)   0.92
    ...
    k_pump = 1.0  # pump gain - due to more powerfull BLDC motor
    S_pump = pi * 0.01 * 0.01  # pump outlet cross section area                 (m^2)
    r_vent = 0.01
    S_vent = pi * r_vent * r_vent
    ...
    dT_gas = 5.0  # additional temperature of the LTA gas due to greenhouse effect
    ...
    pumping_on_off = True  # pumping mode: False - disabled, True - enabled
    ...
    h0 = 6950  # 1120  24000  # initial altitude                 ( < 36000 )   (m)
    h_max = 25000  # the max altitude to stop ascent  ( < 36000 )   (km)
    h_min = 15000  # the min altitude to stop descent ( < 36000 )   (km)
    h_target = 23500
    target_on_off = False
    x_0 = 0  # previous x coordinate (East)                   (m/s)
    y_0 = 0  # previous y coordinate (North)                  (m/s)
    z_0 = h0  # initial/previous z coordinate (altitude)       (m/s)
    x = 0  # current x coordinate  ( -> East  )             (m/s)
    y = 0  # current y coordinate  ( -> North )             (m/s)
    z = h0  # current z coordinate  (altitude)               (m/s)
    ...
    dt = 0.001  # numerical egration step                     (s)
    dt_pr = 1  # cout step                                      (s)
    # Fa                           # buoyant force                                  (N)
    ...
    while (z <= h_max + dh_plus and z > 0):
        ...
        if (((P_atm < P_gas_max) and (m_bl_air <= m_bl_air_1)) or (
                (P_atm >= P_gas_max) and (m_bl_air <= m_bl_air_2)) or (
                (P_atm < P_gas_max) and (m_bl_air == m_bl_air_2))):
            diff_Pmode = 1
            diff_1 = True
        ...
        if ((z > h_target) and (P_bl - P_gas < dP_max_bl) and target_on_off):

            pumping_rate_m_ = rho_atm * (rho_atm * pumping_rate_V * pumping_rate_V + 2 *
                                         (P_atm - P_bl) * S_pump * S_pump / k_pump / k_pump)
            if pumping_rate_m_ < 0:
                pumping_rate_m_ = 0
            else:
                pumping_rate_m = N_pumps_on * sqrt(pumping_rate_m_)
        ...
        if ((z < h_target) and target_on_off):
            vent_on_off = 1
            venting_rate_m = ro_bl_air * S_vent * pow((P_atm/P_bl), (1/k_air)) * \
                             sqrt((P_bl/ro_bl_air)*(2*k_air/(k_air-1))*(1-pow((P_atm/P_bl), ((k_air-1)/k_air))))
